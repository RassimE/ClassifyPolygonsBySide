...
namespace CutOperatorTest
{
	internal static class СlassifyPolygonsBySide
	{
		...
		internal static void СlassifyBySideLinear(List<GeometryBase> inputPolygons, LineString lineString,
		...
		internal static void СlassifyBySideBinary(List<GeometryBase> inputPolygons, LineString lineString,
			double xOrigin, double yOrigin, out GeometryBase geometryLeft, out GeometryBase geometryRight)
		{
			// calculating Hough parameters for each piece of polyline
			LineStat[] lineStats = new LineStat[lineString.Count - 1];
		...
		public static void СlassifyBySide(List<GeometryBase> inputPolygons, LineString lineString,
			double xOrigin, double yOrigin, out GeometryBase geometryLeft, out GeometryBase geometryRight)
		{
			if (lineString.Count > 60)
				СlassifyBySideBinary(inputPolygons, lineString, xOrigin, yOrigin, out geometryLeft, out geometryRight);
			else
				СlassifyBySideLinear(inputPolygons, lineString, xOrigin, yOrigin, out geometryLeft, out geometryRight);
		}
	}
}
